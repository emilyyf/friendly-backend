...
func (h *PasswordHash) ComparePasswordAndHash(password, encodedHash string) (match bool, err error) {
	...
	// Derive the key from the other password using the same parameter
	otherHash := argon2.IDKey([]byte(password), salt, p.iterations, p.memory, p.parallelism, p.keyLength)

	// Check that contents of the hashed passwords are identical. Note
	// that we are using the subtle.ConstantTimeCompare() func for this
